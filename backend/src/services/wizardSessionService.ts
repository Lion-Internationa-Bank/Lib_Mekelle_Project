...
export class WizardSessionService {
  ...
  async validateSession(sessionId: string): Promise<WizardValidationResult> {
    ...
    const ownerData = Array.isArray(session.owner_data) 
      ? (session.owner_data[0] as OwnerData) 
      : (session.owner_data as OwnerData);
    ...
    const parcelData = Array.isArray(session.parcel_data) 
      ? (session.parcel_data[0] as ParcelData) 
      : (session.parcel_data as ParcelData);
      ...
      if (canSelfApprove) {
        ...
        const executionResult = await this.actionExecutionService.executeWizard(
          tx,
          {
            ...
            parcel: session.parcel_data as ParcelData,
            parcel_docs: session.parcel_docs as DocumentData[],
            owners: Array.isArray(session.owner_data) 
              ? (session.owner_data as OwnerData[]) 
              : [session.owner_data as OwnerData],
            owner_docs: session.owner_docs as DocumentData[],
            lease: session.lease_data,
            lease_docs: session.lease_docs as DocumentData[],
  ...
  private async deleteSessionDocuments(sessionId: string): Promise<void> {
    try {
      ...
      // Collect all document references with proper typing
      const allDocs: DocumentData[] = [
        ...(Array.isArray(session.parcel_docs) ? session.parcel_docs as DocumentData[] : []),
        ...(Array.isArray(session.owner_docs) ? session.owner_docs as DocumentData[] : []),
        ...(Array.isArray(session.lease_docs) ? session.lease_docs as DocumentData[] : [])
      ];
      ...
      for (const doc of allDocs) {
        if (doc.file_url) {
          try {
            ...
            console.log(`Deleting document: ${doc.id} from ${step}`);
          } catch (docError) {
