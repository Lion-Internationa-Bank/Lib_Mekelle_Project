...
const ValidationStep = ({ prevStep, onFinish }: FinishStepProps) => {
  const { currentSession, submitForApproval, isLoading } = useWizard();
  ...
  const validateSession = useCallback(async (): Promise<ValidationResult> => {
    ...
    try {
      ...
      if (response.success && response.data) {
        // This covers both:
        // { success: true, data: { valid: true, missing: [] } }
        // and { success: true, data: { valid: false, missing: [...] } }
        if (
            ...
            {currentSession.parcel_data ? (
              <div className="space-y-2">
                <div>
                  ...
                  {currentSession.parcel_data.upin}
                </div>
                <div>
                  <span className="font-medium">File Number:</span>{" "}
                  {currentSession.parcel_data.file_number}
                </div>
                <div>
                  <span className="font-medium">Area:</span>{" "}
                  {currentSession.parcel_data.total_area_m2} m²
                </div>
                <div>
                  <span className="font-medium">Land Use:</span>{" "}
                  {currentSession.parcel_data.land_use}
                </div>
                <div>
                  <span className="font-medium">Tenure:</span>{" "}
                  {currentSession.parcel_data.tenure_type}
                </div>
              </div>
            ) : (
                ...
                {Array.isArray(currentSession.owner_data)
                  ...
                  : (
                    <>
                      <div>
                        <span className="font-medium">National ID:</span>{" "}
                        {currentSession.owner_data.national_id}
                      </div>
                      <div>
                        <span className="font-medium">Phone:</span>{" "}
                        {currentSession.owner_data.phone_number}
                      </div>
          ...
          {currentSession.lease_data && (
            <div className="md:col-span-2 bg-purple-50 rounded-xl p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <span className="font-medium">Total Amount:</span> ETB{" "}
                  {currentSession.lease_data.total_lease_amount?.toLocaleString()}
                </div>
                <div>
                  <span className="font-medium">Period:</span>{" "}
                  {currentSession.lease_data.lease_period_years} years
                </div>
                <div>
                ...
                <div>
                  <span className="font-medium">Price/m²:</span> ETB{" "}
                  {currentSession.lease_data.price_per_m2?.toLocaleString()}
                </div>
                <div>
                  <span className="font-medium">Payment Term:</span>{" "}
                  {currentSession.lease_data.payment_term_years} years
                </div>
